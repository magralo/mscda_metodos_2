adalasso2=function (X, y, prices=1,k = 10, use.Gram = TRUE, both = TRUE, intercept = TRUE) 
{
  colnames(X) = 1:ncol(X)
  n <- length(y)
  cv.adalasso <- NULL
  ...
  if (both == TRUE) {
    ...
    for (i in seq(k)) {
      ...
      weights <- 1/abs(coef.lasso[abs(coef.lasso) > 0]) #### MODIFICAR CON EL PRECIO
      #print(length(weights))
      prices_red=prices[abs(coef.lasso) > 0]
      weights <- prices_red/weights 
      #print(length(weights))
      if (length(weights) == 0) {
        residmat[, i] <- mean((mean(ytrain) - ytest)^2)
      }
      if (length(weights) == 1) {
        residmat[, i] = mean((ytest - my.lars$intercept - 
                                Xtest %*% coef.lasso)^2)
      }
      if (length(weights) > 1) {
        ...
        fit <- glmnet(XXtrain, ytrain, type.gaussian = type, 
    ...
    weights <- 1/abs(coefficients.lasso[abs(coefficients.lasso) > 
                                          0])### modificar tambien aqui
    #print(length(weights))
    prices_red=prices[abs(coefficients.lasso) >0]
    weights <- prices_red/weights 
    #print(length(weights))
